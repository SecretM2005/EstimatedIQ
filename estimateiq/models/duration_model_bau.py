"""
XGBoost-Laufzeitschätzungsmodell für Bauprojekte.

Zielvariable: log(dauer_tage)
Features:     BERT-Embeddings + tabularische Geo/CPV/BBSR-Features
Ausgabe:      Vorhersage in Tagen (rücktransformiert via exp)

Trainiert UNABHÄNGIG vom Kostenmodell – keine zweistufige Pipeline.

Artefakte (unter models/):
  duration_model_bau.pkl      – trainiertes XGBoost-Modell
  duration_encoders_bau.pkl   – OrdinalEncoder
  duration_importance_bau.png – Feature-Importance-Plot (Top 15)
"""
...
MODELL_PKL  = MODELL_DIR / "duration_model_bau.pkl"
ENCODER_PKL = MODELL_DIR / "duration_encoders_bau.pkl"
PLOT_PNG    = MODELL_DIR / "duration_importance_bau.png"
...
def _extrahiere_flaeche(text: str) -> float:
    for pat in [r"(\d[\d.]*)\s*[,.]?\d*\s*(?:m\s*[²2]|qm|Quadratmeter)"]:
...
def _extrahiere_einheiten(text: str) -> float:
    m = re.search(r"(\d+)\s*(?:Wohneinheit|Wohnung|Appartement|WE\b)", text, re.IGNORECASE)
    if m:
        try:
            val = float(m.group(1))
            if 1 <= val <= 10_000:
...
def train(
    ...
    """
    Trainiert XGBoost-Laufzeitmodell für Bauprojekte.
    ...
    Returns:
        dict mit rmse_tage, r2, mdape, n_train, n_test, best_iteration
    """
    logger.info("[Training] Starte Bau-Laufzeitmodell-Training...")

    df = _feature_engineering(df)
    df_sauber = df[df["dauer_tage"].notna()].reset_index(drop=True)

    if embeddings is not None:
        embeddings = embeddings[df["dauer_tage"].notna().values]

    n = len(df_sauber)
    logger.info("[Training] %d Zeilen mit dauer_tage.", n)

    if n < 50:
        raise ValueError(f"Zu wenig Trainingsdaten: {n} Zeilen (Minimum: 50).")

    y_log = np.log1p(df_sauber["dauer_tage"].astype(float).values)
    ...
    mdape     = float(np.median(np.abs(y_pred_tage - y_true_tage) / np.maximum(y_true_tage, 1)) * 100)

    logger.info(
        "[Training] RMSE: %.1f Tage | R²: %.4f | MdAPE: %.1f%% | Best Iter: %d",
        rmse_tage, r2, mdape, modell.best_iteration,
    )
...
def predict(df: pd.DataFrame, embeddings: np.ndarray | None = None) -> np.ndarray:
    """Sagt Projektlaufzeit in Tagen voraus."""
...
def _erstelle_importance_plot(modell: XGBRegressor, mit_bert: bool = False) -> None:
    ...
    farben = ["#065f46" if i == 0 else "#6ee7b7" for i in range(len(werte))]
    ...
    ax.set_title("Feature Importance – Bau Duration Model (Top 15)", fontsize=13, fontweight="bold")
...
if __name__ == "__main__":
    ...
    print(f"\n{'─'*46}")
    print("  Bau Duration Model – Ergebnis")
    print(f"{'─'*46}")
    print(f"  RMSE:          {metriken['rmse_tage']:>12.1f} Tage")
