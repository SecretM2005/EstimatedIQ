"""
Spezialisiertes Laufzeit-Modell für grosse Projekte (180–730 Tage).

Grosse Projekte: Enterprise-Teams, SAP/ERP, 200+ Nutzer, komplexe Integrationen.
Trainingsdaten: Projekte mit 180 <= dauer_tage <= 730 (primär TED-Daten).

Artefakte:
  models/duration_model_gross.pkl   – {modell, tfidf, svd}
  models/duration_encoders_gross.pkl – OrdinalEncoder
"""
...
DAUER_MIN    = 180
DAUER_MAX    = 730
KALIBRIERUNG = 0.65   # TED-Daten: Vergabe-Puffer und Betriebsphasen abziehen
MODELL_DIR   = Path("models")
MODELL_PKL   = MODELL_DIR / "duration_model_gross.pkl"
ENCODER_PKL  = MODELL_DIR / "duration_encoders_gross.pkl"
...
def train(df: pd.DataFrame, test_anteil: float = 0.20) -> dict:
    """
    Trainiert spezialisiertes Laufzeit-Modell für grosse Projekte.
